...
TIER_A_STRUCTURAL = [
    "Header_Length", "Tot sum", "Tot size", "Min", "Max",
    "AVG", "Std", "Variance", "Protocol Type", "Number",
]
TIER_B_TIMING = ["Rate", "IAT"]
TIER_C_SESSION_STATE = [
    "syn_flag_number", "rst_flag_number", "psh_flag_number", "ack_flag_number",
    "syn_count", "ack_count", "fin_count", "rst_count",
]

FEATURE_TIER_MAP = {}
FEATURE_TIER_MAP.update({f: "A" for f in TIER_A_STRUCTURAL})
FEATURE_TIER_MAP.update({f: "B" for f in TIER_B_TIMING})
FEATURE_TIER_MAP.update({f: "C" for f in TIER_C_SESSION_STATE})
...
def restrict_to_tiers(feature_list, allowed_tiers):
    # Keeps only features whose stability tier is in allowed_tiers.
    # Features with no tier assignment .
    return [f for f in feature_list if FEATURE_TIER_MAP.get(f) in allowed_tiers]
...
#  Heavy model (XGBoost, GPU, sample-weighted, Optuna-tuned)
...
def run_pipeline(sample_frac_ciciot=0.02, optuna_trials=20, min_per_class=1000):
    ...
    heavy_features = resolve_feature_set(train, HEAVY_FEATURES, "HEAVY_FEATURES")
    lite_features = resolve_feature_set(train, LITE_FEATURES, "LITE_FEATURES")
    ...
    results = {}
    for track_name, allowed_tiers in [("legacy", ("A", "B", "C")), ("robust", ("A", "B"))]:
        print(f"TRACK: {track_name.upper()}")
        
        heavy_candidates = restrict_to_tiers(
                    resolve_feature_set(train, HEAVY_FEATURES, f"HEAVY[{track_name}] candidates"),
                    allowed_tiers,
                )
        lite_candidates = restrict_to_tiers(
                    resolve_feature_set(train, LITE_FEATURES, f"LITE[{track_name}] candidates"),
                    allowed_tiers,
                )
        
        heavy_features, _ = select_features_for_track(train, heavy_candidates, heavy_top_k=None)
        _, lite_features = select_features_for_track(train, lite_candidates, lite_top_k=6)
        
        print(f"HEAVY[{track_name}] final features ({len(heavy_features)}): {heavy_features}")
        print(f"LITE[{track_name}] final features ({len(lite_features)}): {lite_features}")
        ...
        heavy_results = evaluate_model(heavy_model, test_h[heavy_features], test_h["y"],
                                        label_names=label_encoder.classes_, model_name=f"heavy_xgboost_{track_name}")
        print("\nLite model (CICIoT2023 test):")
        lite_results = evaluate_model(lite_model, test_l[lite_features], test_l["y"],
                                    label_names=label_encoder.classes_, model_name=f"lite_decision_tree_{track_name}")
        ...
        # Export to ONNX
        heavy_onnx_path = get_unique_path(OUTPUT_DIR / f"heavy_{track_name}_xgboost.onnx")
        lite_onnx_path = get_unique_path(OUTPUT_DIR / f"lite_{track_name}_decision_tree.onnx")
        try:
            export_heavy_to_onnx(heavy_model, len(heavy_features), str(heavy_onnx_path))
        except Exception as e:
            print(f"[WARN] Heavy[{track_name}] ONNX export failed: {e}")
            heavy_onnx_path = None
        try:
            export_lite_to_onnx(lite_model, len(lite_features), str(lite_onnx_path))
        except Exception as e:
            print(f"[WARN] Lite[{track_name}] ONNX export failed: {e}")
            lite_onnx_path = None
...
if __name__ == "__main__":
    ...
    results = run_pipeline(sample_frac_ciciot=0.02, optuna_trials=10, min_per_class=1000)
    print(results)

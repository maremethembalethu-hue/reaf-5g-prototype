...
# MODEL_DIR is now a STABLE, run-name-independent location. Deploying a new
# training run means copying that run's output files INTO this directory
# directly (overwriting what's there), not pointing this constant at a
# run-specific subfolder name.
MODEL_DIR = os.getenv("MODEL_ARTIFACTS_DIR",
                       os.path.join(os.path.dirname(__file__), "trained_models/three_stage_DNS"))
...
def _run_stage2_stage3(sessions, scaled):
    ...
    if family == "Flood":
        ...
        margin = abs(float(p3[ddos_pos]) - float(p3[dos_pos]))
        ...
        debug["stage3"] = {"margin": margin, "confident": bool(margin >= MIN_MARGIN)}
